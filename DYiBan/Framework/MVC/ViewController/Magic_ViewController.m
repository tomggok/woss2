...
{
    MagicViewController *_owner;
}
@property (nonatomic, assign)MagicViewController *owner;
...
- (void)setFrame:(CGRect)frame
{
    ...
    if (_owner) {
        [_owner sendViewSignal:MagicViewController.LAYOUT_VIEWS];
    }
}
...
@implementation UIView(MagicViewController)
- (MagicViewController *)board
{
    UIView *topView = self;
    ...
    while (topView.superview) {
        ...
        if ([topView isKindOfClass:[UIWindow class]]) {
            break;
        }else if ([topView isKindOfClass:[MagicView class]])
        {
            break;
        }
    }
    ...
    if (controller && [controller isKindOfClass:[MagicViewController class]]) {
...
+ (MagicViewController *)vcalloc
{
    return [[(MagicViewController *)[MagicRuntime allocByClass:[self class]] init] autorelease];
}
- (id)init
{
    self = [super init];
...
- (void)changeStateDisAppeared
{
    if (STATE_VIEW_DISAPPEARED != _int_state) {
        _int_state = STATE_VIEW_DISAPPEARED;
        [self sendViewSignal:MagicViewController.DID_DISAPPEAR];
    }
...
- (void)changeStateDisAppearing
{
    if (STATE_VIEW_DISAPPEARING != _int_state) {
        ...
        [self sendViewSignal:MagicViewController.WILL_DISAPPEAR];
    }
...
- (void)changeStateAppeared
{
    if (STATE_VIEW_APPEARED != _int_state) {
        ...
        [self sendViewSignal:MagicViewController.DID_APPEAR];
    }
}
...
- (void)changeStateAppearing
{
    if (STATE_VIEW_APPEARING != _int_state) {
        ...
        [self postNotification:[MagicViewController NOTIFICATIONWILLAPPEAR] withObject:self];
        [self sendViewSignal:MagicViewController.WILL_APPEAR];
    }
}
...
- (void)loadView
{
    ...
    MagicView *boardView = [[MagicView alloc] initWithFrame:boardViewBound];
...
- (void)createViews
{
    ...
    if (!_bool_viewBuilt) {
        [self sendViewSignal:MagicViewController.CREATE_VIEWS];
        [self sendViewSignal:MagicViewController.LAYOUT_VIEWS];
...
{
    if (_bool_viewBuilt) {
        [self sendViewSignal:MagicViewController.DELETE_VIEWS];
...
{
    if (!_bool_dataLoaded) {
        [self sendViewSignal:MagicViewController.LOAD_DATAS];
...
{
    if (_bool_dataLoaded) {
        [self sendViewSignal:MagicViewController.FREE_DATAS];
        _bool_dataLoaded = NO;
    }
}
...
{
    [self sendViewSignal:MagicViewController.ENTERBACKGROUND];
}
- (void)enterForeground
{
    [self sendViewSignal:MagicViewController.ENTERFOREGROUND];
}
//////////////////
- (MagicNavigationController *)drNavigationController
{
    if (self.navigationController) {
        return (MagicNavigationController *)self.navigationController;
    }else
    {
        return nil;
    }
...
- (void)didReceiveMemoryWarning
{
    ...
    float sysVer = [MagicDevice sysVersion];
...
- (void)setVCBackAnimation:(MagicVCBackType)animationType canBackPageNumber:(NSInteger)number
{
