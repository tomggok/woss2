...
@implementation UIView(MagicViewNavigation)
- (MagicNavigationController *)stack
{
    UINavigationController *controller = [self navigationController];
    if (controller && [controller isKindOfClass:[MagicNavigationController class]]) {
        return (MagicNavigationController *)controller;
    }else
    {
        return nil;
    }
...
@interface MagicNavigationBar : UINavigationBar
{
...
@end
@implementation MagicNavigationBar
...
+ (MagicNavigationController *)stack
{
    return [[[MagicNavigationController alloc] initWithName:nil andFirstBoardClass:nil] autorelease];
}

+ (MagicNavigationController *)stack:(NSString *)name
{
    return [[[MagicNavigationController alloc] initWithName:name andFirstviewController:nil] autorelease];
}

+ (MagicNavigationController *)stack:(NSString *)name firstViewClass:(Class)clazz
{
    MagicViewController *viewController = [[(MagicViewController *)[MagicRuntime allocByClass:clazz] init] autorelease];
    return [[MagicNavigationController alloc] initWithName:name andFirstviewController:viewController];
}

+ (MagicNavigationController *)stack:(NSString *)name firstviewController:(MagicViewController *)viewController
{
    return [[[MagicNavigationController alloc] initWithName:name andFirstviewController:viewController] autorelease];
}

+ (MagicNavigationController *)stackWithFirstViewControllerClass:(Class)clazz
{
    return [MagicNavigationController stack:nil firstViewClass:clazz];
}

+ (MagicNavigationController *)stackWithFirstViewController:(MagicViewController *)viewController
{
    return [MagicNavigationController stack:nil firstviewController:viewController];
}
...
- (NSArray *)allviewControllers
{
    ...
    for (UIViewController *controller in self.viewControllers) {
        if ([controller isKindOfClass:[MagicViewController class]]) {
...
- (MagicViewController *)topStackViewController
{
    UIViewController *controller = self.topViewController;
    if (controller && [controller isKindOfClass:[UIViewController class]]) {
        MagicViewController *viewController = (MagicViewController *)controller;
...
- (UIViewController *)popViewControllerAnimated:(BOOL)animated
{
    ...
    if (count > 0)
    {
        [self sendViewSignal:[MagicViewController VCBACKSUCCESS] withObject:nil from:nil target:[self.viewControllers objectAtIndex:count]];
    }
...
//跳转上一个页面的动画
- (NSArray *)animationPopView:(MagicViewController *)viewController animated:(BOOL)animated animationType:(NSInteger)type jumpType:(NSInteger)jumpType
{
    NSArray *result = nil;
...
//跳转下一个页面的动画效果
- (void)animationToNextPage:(MagicViewController *)viewController animated:(BOOL)animated animationType:(NSInteger)type jumpType:(NSInteger)jumpType
{
...
//是什么跳转方式
- (NSArray *)naviPushOrPop:(NSInteger)jumpType animated:(BOOL)animated popToViewController:(MagicViewController *)viewController
{
    switch (jumpType) {
...
- (BOOL)existsViewController:(MagicViewController *)board
{
    for (UIViewController *controller in self.viewControllers)
    {
        if ([controller isKindOfClass:[MagicNavigationController class]] && (MagicViewController *)controller == board) {
            return YES;
        }
    }
    return NO;
...
- (MagicViewController *)getViewController:(Class)clazz
{
    for (UIViewController *controller in self.viewControllers) {
        if ([controller isKindOfClass:clazz]) {
            return (MagicViewController *)controller;
        }
    }
    return nil;
}
...
- (void)enterBackground
{
    for (UIViewController *viewController in self.viewControllers) {
        if ([viewController isKindOfClass:[MagicViewController class]]) {
            MagicViewController *vc = (MagicViewController *)viewController;
            [vc enterBackground];
        }
    }
}
...
- (void)setBarBackgroundImage:(UIImage *)image
{
    ...
    if (navBar && [navBar isKindOfClass:[MagicNavigationBar class]])
    {
        [(MagicNavigationBar *)navBar setBackgroundImage:image];
    }
}
...
- (void)setBarBackgroundColor:(UIColor *)color
{
    ...
    if (navBar && [navBar isKindOfClass:[MagicNavigationBar class]])
    {
        [(MagicNavigationBar *)navBar setBackgroundImage:[UIImage imageWithColor:color]];
    }
}
...
{
    [super loadView];
    MagicNavigationBar *bar = [[MagicNavigationBar alloc] init];
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    if (self.topViewController && [self.topViewController isKindOfClass:[MagicViewController class]]) {
        MagicViewController *baseVC = (MagicViewController *)self.topViewController;
...
- (void)viewDidDisappear:(BOOL)animated
{
    ...
    if (self.topViewController && [self.topStackViewController isKindOfClass:[MagicViewController class]]) {
        MagicViewController *baseVC = (MagicViewController *)self.topViewController;
