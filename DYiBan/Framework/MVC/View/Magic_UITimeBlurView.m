...
#import "Magic_UITimeBlurView.h"
#import "UIImage+MagicCategory.h"

@interface MagicTimeBlurViewManager : NSObject

@property (nonatomic, retain)NSMutableArray *views;

+ (id)sharedManager;
- (void)registerView:(MagicUITimeBlurView *)view;
- (void)deregiststerView:(MagicUITimeBlurView *)view;

@end

@interface MagicUITimeBlurView ()
{
}
...
@implementation MagicTimeBlurViewManager

static MagicTimeBlurViewManager *shared = nil;
+ (id)sharedManager
{
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        shared = [[MagicTimeBlurViewManager alloc] init];
    });
...
- (void)refresh
{
    ...
    for (MagicUITimeBlurView *view in _views)
    {
...
@implementation MagicUITimeBlurView
...
- (void)setRenderStatic:(BOOL)renderStatic
{
    ...
    if (renderStatic)
    {
        [[MagicTimeBlurViewManager sharedManager] deregiststerView:self];
    }else
    {
        [[MagicTimeBlurViewManager sharedManager] registerView:self];
    }
...
- (void)toggleBlurViewsInView:(UIView*)view hidden:(BOOL)hidden alpha:(CGFloat)originalAlpha
{
    ...
    {
        if ([subView isKindOfClass:[MagicUITimeBlurView class]])
        {
