...
@class DragonUIUpdateView;
@protocol DragonUIUpdateViewDelegate <NSObject>

@optional
- (void)slimeRefreshStart:(DragonUIImageView *)refreshView;

@end
typedef void(^DUpdateBlock)(DragonUIUpdateView *sender);
...
@interface DragonUIUpdateView : UIView
{
    DragonUIImageView *_refleshView;
    DSlimeView        *_slime;
}
...
@property (nonatomic, retain, readonly)DragonUIImageView *refleshView;

@property (nonatomic, copy)DUpdateBlock block;
@property (nonatomic, assign)id<DragonUIUpdateViewDelegate> delegate;
