...
#import "Magic_UISearchBar.h"
#import "UITableView+property.h"
#import "NSDictionary-MutableDeepCopy.h"
#import "UIView+MagicCategory.h"

@implementation MagicUISearchBar
...
- (void)layoutSubviews {
...

    {
        ...
        if (searchField) {
        // 改输入字体颜色
        searchField.textColor = [MagicCommentMethod colorWithHex:@"333333"];

        // 改 placeholde 颜色
        [searchField setValue:[MagicCommentMethod colorWithHex:@"aaaaaa"] forKeyPath:@"_placeholderLabel.textColor"];
...
{
    if (!_bt_Shade) {
        _bt_Shade=[[MagicUIButton alloc]initWithFrame:CGRectMake(0, self.frame.origin.y+self.frame.size.height, screenShows.size.width, screenShows.size.height)];
...
{
    if (!_bt_cancel) {
        _bt_cancel=[[MagicUIButton alloc]initWithFrame:CGRectMake(CGRectGetMaxX(self.frame), /*CGRectGetMinY(self.frame)*/ 0, normalImg.size.width/2, normalImg.size.height/2)];
        ...
        [_bt_cancel addSignal:[MagicUIButton TOUCH_UP_INSIDE] forControlEvents:UIControlEventTouchUpInside object:self];
        ...
        if (bt)
        {
            switch (bt.tag) {
                ...
                case k_tag_fadeBt:
                {
                    ...
                    [self sendViewSignal:[MagicUISearchBar CANCEL] withObject:self];
                }
                    break;
                default:
                    break;
...
- (BOOL)searchBarShouldBeginEditing:(UISearchBar *)searchBar {
    ...
    [self sendViewSignal:[MagicUISearchBar BEGINEDITING] withObject:self];
...
{
    [self sendViewSignal:[MagicUISearchBar SEARCH] withObject:self];
}
...
-(void)searchBar:(UISearchBar *)searchBar textDidChange:(NSString *)searchText{
    ...
    [self sendViewSignal:[MagicUISearchBar CHANGEWORD] withObject:searchText];
