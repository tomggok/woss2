...
#import "Magic_UILabelImgView.h"

@interface MagicUILabel ()
{
...
- (MagicUILabelC)COLOR
{
    MagicUILabelC block = ^MagicUILabel * (id first,...)
    {
        va_list args;
        va_start(args, first);
...
- (MagicUILabelC)FONT
{
    ...
    MagicUILabelF block = ^MagicUILabel * (id first,...)
    {
        va_list args;
        va_start(args, first);
...
- (MagicUILabelAI)IMGA
{
    MagicUILabelAI block = ^MagicUILabel * (id first,...)
    {
        va_list args;
        va_start(args, first);
...
- (MagicUILabelI)IMG
{
    MagicUILabelI block = ^MagicUILabel * (id first,...)
    {
        va_list args;
        va_start(args, first);
...
- (MagicUILabelCl)CLICK
{
    MagicUILabelCl block = ^MagicUILabel * (id first, ...)
    {
        va_list args;
        va_start(args, first);
...
- (void)drawIMG:(CGContextRef)context frame:(CTFrameRef)frame;
{
    if ([[_dict_selfImg allValues] count] > 0)
    {
        for (int i = 0; i < [[_dict_selfImg allKeys] count]; i++)
        {
            ...
            MagicUILabelImgView *img = [[MagicUILabelImgView alloc] initWithFrame:CGRectMake(0, 0, self.bounds.size.width, self.bounds.size.height)];
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    [self sendViewSignal:[MagicUILabel TOUCHESBEGAN] withObject:dict];
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    [self sendViewSignal:[MagicUILabel TOUCHESMOVED] withObject:dict];
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    [self sendViewSignal:[MagicUILabel TOUCHESENDED] withObject:dict];
...
- (void)touchesCancelled:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    [self sendViewSignal:[MagicUILabel TOUCHESCANCEL] withObject:dict];
