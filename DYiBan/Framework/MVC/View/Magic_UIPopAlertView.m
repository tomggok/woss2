...
@interface MagicUIPopAlertView ()
{
    //alertMode
    UIView *_alertView;//弹出view
    UIView *_childAlertView;
    MagicUILabel *_label;//view上的label
    MagicUIImageView *_indicatorImgView;//句话view
...
@end
@implementation MagicUIPopAlertView
...
- (void)initAlertView
{
    ...
    if (!_label)
    {
        _label = [[MagicUILabel alloc] initWithFrame:CGRectMake(10, 10, 20*10, 20*2)];
...
#pragma mark - method
//设置popview的类型
- (void)setMode:(MagicpopViewType)mode
{

    _mode = mode;
    
    if (_mode == MagicPOPALERTVIEWNOINDICATOR || _mode == MagicPOPALERTVIEWINDICATOR)
    {
        [self initAlertView];
        if (_mode == MagicPOPALERTVIEWINDICATOR)
        {
            if (_indicatorMode == -1)
            {
...
//设置菊花位置
- (void)setIndicatorMode:(MagicpopAlertIndicatorType)indicatorMode
{
    [self initViewFrame];
    ...
    self.mode = MagicPOPALERTVIEWINDICATOR;
...
- (void)setText:(NSString *)text
{
    if (!_label)
    {
        [self setMode:MagicPOPALERTVIEWNOINDICATOR];
    }
    [_label setText:text];
...
//                             [UIImage imageNamed:@"gif12.png"],nil];
        ...
        _indicatorImgView = [[MagicUIImageView alloc] initWithFrame:CGRectMake(8, 0, 16, 16)];
...
- (void)alertViewHidden
{
    [UIView animateWithDuration:1.f animations:^{
        ...
        [_alertView setHidden:YES];
        [self sendViewSignal:[MagicUIPopAlertView POPALERTHIDDEN] withObject:nil from:self target:_delegate];
    }];
}
