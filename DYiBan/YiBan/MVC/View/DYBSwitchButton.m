...
@interface DYBSwitchButton () <UIGestureRecognizerDelegate>
@property (nonatomic, retain) KLSwitchTrack* track;
@property (nonatomic, retain) KLSwitchThumb* thumb;
...
@implementation DYBSwitchButton

- (void)dealloc {
    
    RELEASE(self.onImage);
    RELEASE(self.offImage);
    RELEASE(self.onTintColor);
    RELEASE(self.tintColor);
    RELEASE(self.thumbTintColor);
    RELEASE(self.contrastColor);
    RELEASE(self.thumbBorderColor);
    RELEASEALLSUBVIEW(self.track);
    RELEASE(self.thumb);
    [super dealloc];
}
...
- (void)drawRect:(CGRect)rect
{
    ...
    [_thumb.layer setBorderColor: [UIColor colorWithWhite: 0.9f alpha:1.0f].CGColor];
...
@property (nonatomic, retain) UIView* contrastView;
...
- (void)dealloc {
    ...
    RELEASEALLSUBVIEW(self.contrastView);
    RELEASE(self.onView);
    [super dealloc];
}
