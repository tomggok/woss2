...
//        RELEASE(btnResgin);
        ...
        UIImageView *imageViewIcon = [[UIImageView alloc]initWithFrame:CGRectMake((320 - 0 - 60)/2 , 70.0f, 60.0f, 60.0f)];
        [imageViewIcon setImage:[UIImage imageNamed:@"log"]];
        [self.view addSubview:imageViewIcon];
        [imageViewIcon release];
        
        _phoneInputNameR = [[DYBInputView alloc]initWithFrame:CGRectMake((320-INPUTWIDTH)/2, 0 + 20 + 150, INPUTWIDTH, INPUTHEIGHT) placeText:@"用户名" textType:0];
...
//        input_bg
        ...
        UIImageView *imageViewName = [[UIImageView alloc]initWithFrame:CGRectMake((320-imageK.size.width/2)/2, 100 + 0, imageK.size.width/2, imageK.size.height/2 )];
        ...
        _phoneInputName = [[DYBInputView alloc]initWithFrame:CGRectMake(60, 0, INPUTWIDTH, INPUTHEIGHT) placeText:@"用户名" textType:0];
        ...
        UIImageView *imageViewName2 = [[UIImageView alloc]initWithFrame:CGRectMake(75, 0 +INPUTHEIGHT  + 40, INPUTWIDTH, INPUTHEIGHT)];
...
//        RELEASE(imageViewName2);
        ...
        UIButton *btnBack= [[UIButton alloc]initWithFrame:CGRectMake((320 - 44)/2, 200, 44, 44)];
...
//        [self addlabel_title:@"登陆" frame:btnBack.frame view:btnBack];
        ...
        UIImage *imageA = [UIImage imageNamed:@"加号"];
        UIButton *btnADD = [[UIButton alloc]initWithFrame:CGRectMake(180.0f, 10.0f, imageA.size.width/2, imageA.size.height/2)];
        [btnADD setImage:imageA forState:UIControlStateNormal];
        [btnADD addTarget:self action:@selector(doDD) forControlEvents:UIControlEventTouchUpInside];
        [imageViewName addSubview:btnADD];
        [btnADD release];
        
        UIImage *image11 = [UIImage imageNamed:@"？"];
        UIButton *btnADD1 = [[UIButton alloc]initWithFrame:CGRectMake(184.0f, 50.0f, image11.size.width/2, image11.size.height/2)];
        [btnADD1 setImage:[UIImage imageNamed:@"？"] forState:UIControlStateNormal];
        [btnADD1 addTarget:self action:@selector(doDD1) forControlEvents:UIControlEventTouchUpInside];
        [imageViewName addSubview:btnADD1];
        [btnADD1 release];
    }
...
-(void)doDD{
    ...
    WOSRegisterViewController *registe = [[WOSRegisterViewController alloc]init];
    [self.drNavigationController pushViewController:registe animated:YES];
    RELEASE(registe);
